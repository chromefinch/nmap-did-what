...
echo "Script to bulk import xml's using a for loop & the nmap-to-sqlite-NG.py script" 
echo "(Please run from folder containing nmap-to-sqlite-NG.py)"
...
for file in "$folder"/*.xml; do
  if [ -f "$file" ]; then
    ...
    if [[ "$file" == *phase4* ]] || [[ "$file" == *phase5* ]]; then
      ...
      python3 nmap-to-sqlite-NG.py "$file"
      
      if [ $? -ne 0 ]; then
        echo "Error: python3 nmap-to-sqlite-NG.py '$file' failed."
      fi
    fi
  fi
done
