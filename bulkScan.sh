...
pingSweepdefault="Y"
read -p "Skip Ping Sweep? (Y/n): " pingSweepq
pingSweep=${pingSweepq:-$pingSweepdefault}
Phase3default="-p-"
...
nmap -sS -T4 --max-retries 1 --max-rtt-timeout 300ms --host-timeout 3m --max-scan-delay 5 --min-rate 800 -Pn -n \
     -iL "${HOST_LIST_FILE}" \
     --top-ports $topPorts \
...
case $pingSweep in
    [nN]) pingSweepFunct ;;
    *) print_yellow "[+] Skipping Ping Sweep" ;;
esac
